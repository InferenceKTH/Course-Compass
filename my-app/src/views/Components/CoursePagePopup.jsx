...
function CoursePagePopup({
	...
	return (
		...
		>
			<div
				...
				className="bg-gray-100/90  h-full w-full flex flex-col overflow-auto"
				onClick={(e) => {
					e.stopPropagation();
					setShowOverlay(true);
				}}
  ...
  className={`inline-flex items-center px-4 py-2 gap-2 rounded-lg
    ...
    ${favouriteCourses.some((fav) => fav.code === course.code)
      ? 'bg-violet-600 hover:bg-violet-700 border-2 border-violet-700 hover:border-violet-800 text-white'
      : 'bg-violet-500 hover:bg-violet-600 border-2 border-violet-600 hover:border-violet-700 text-white'
    }`}
  onClick={(e) => {
    e.stopPropagation();
